...
from opencode_client import AsyncOpenCodeClient, FileNode, OpenCodeApiError, OpenCodeTransportError
...
async def read_one(base_url: str, directory: str, path: str) -> None:
    ...
    if content.type == "text":
        ...
        mime = content.mime_type or "unknown"
        print(f"== {path} (binary, {content.encoding}, mime={mime}) ==")
...
async def main(base_url: str, directory: str, path: str, read_path: str | None) -> None:
    ...
    # read 单独开一个 client 演示；真实程序里应该复用同一个 client（见 client/client_reuse.py）。
...
def cli() -> None:
    """解析参数并运行 main。"""
    parser = argparse.ArgumentParser(description="浏览服务端工作区的文件系统")
